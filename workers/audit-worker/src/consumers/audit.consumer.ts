...
export class AuditConsumer implements OnModuleInit {
    ...
    async onModuleInit(): Promise<void> {
        ...
        await this.queueConsumer.consume(this.QUEUE_NAME, async (msg: ConsumeMessage) => this.handleMessage(msg));
    }
    ...
    private async handleMessage(msg: ConsumeMessage): Promise<void> {
        try {
            ...
            this.logger.error(`[AuditConsumer] Failed to process audit message: ${error.message}`, error.stack);
            throw error;
        }
    }
}
