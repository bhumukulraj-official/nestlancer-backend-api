...
describe('WebhookWorkerService', () => {
                ...
                WebhookWorkerService,
                {
                    ...
                    useValue: {
                        get: jest.fn().mockImplementation((token: any) => {
                            if (token === PaymentCapturedHandler) return mockHandlers['razorpay:payment.captured'];
    ...
    describe('dispatch', () => {
        it('should find matching handler and delegate', async () => {
            mockHandlers['razorpay:payment.captured'].canHandle.mockImplementation((p, e) => p === 'razorpay' && e === 'payment.captured');
        ...
        it('should throw ResourceNotFoundException if no handler found', async () => {
            service.onModuleInit();
            await expect(service.dispatch('unknown', 'event', {})).rejects.toThrow(ResourceNotFoundException);
        });
    });
});
