...
export class PushProviderService implements OnModuleInit {
    ...
    onModuleInit(): void {
        ...
        if (publicKey && privateKey && subject) {
            ...
            this.logger.log('[PushProvider] Successfully configured VAPID details for Web Push delivery.');
        } else {
            this.logger.warn('[PushProvider] VAPID details missing from configuration. Web Push delivery will be disabled.');
        }
    }
    ...
    async sendNotification(subscription: any, payload: PushPayload): Promise<boolean> {
        try {
            ...
            if (error.statusCode === 410 || error.statusCode === 404) {
                this.logger.warn(`[PushProvider] Delivered failed: Subscription expired or invalid (HTTP ${error.statusCode})`);
                return false;
            }
            this.logger.error(`[PushProvider] Internal error during delivery: ${error.message}`, error.stack);
            throw error;
        }
    }
}
