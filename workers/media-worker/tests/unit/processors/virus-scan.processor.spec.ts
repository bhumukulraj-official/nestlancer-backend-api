  ...
  describe('scanFile', () => {
    ...
    it('should handle virus found', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        'VIRUS DETECTED in testfile.txt: EICAR-Test-Signature',
      );
    });
    ...
    it('should handle clamav scan error', async () => {
      ...
      expect(result).toEqual({
        isInfected: false,
        details: 'Scan failed, assuming clean for now (fallback policy needed)',
      });
    ...
    it('should handle caught error during processing', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        'Error during virus scan stage: S3 Download Failed',
      );
