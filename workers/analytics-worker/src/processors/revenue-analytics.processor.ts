...
export class RevenueAnalyticsProcessor {
    ...
    async process(period: Period): Promise<void> {
        ...
        const revenueByMethod = await this.aggregationService.aggregate('payment', ['method'], { amount: 'sum' });
        const totalPayments = await this.aggregationService.aggregate('payment', ['status'], { id: 'count', amount: 'sum' });

        const data = {
            methods: revenueByMethod.reduce((acc, curr) => ({ ...acc, [curr.method]: curr._sum.amount }), {}),
            statuses: totalPayments.reduce((acc, curr) => ({
                ...acc,
