...
export async function loadSecrets(): Promise<Record<string, string>> {
    ...
    try {
        ...
        logger.log(`Secrets loaded for environment: ${infisicalEnv} (${Object.keys(secrets).length} secrets)`);
    } catch (error) {
        logger.error('Failed to load secrets from Infisical', error instanceof Error ? error.message : String(error));
    }
