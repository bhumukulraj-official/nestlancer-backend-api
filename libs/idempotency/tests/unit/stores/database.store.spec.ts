...
describe('DatabaseIdempotencyStore', () => {
    ...
    it('should return null and warn if prisma is not set', async () => {
        ...
        expect(loggerSpy).toHaveBeenCalledWith('PrismaService not available for DatabaseIdempotencyStore');
    });
    ...
    it('should upsert record in database', async () => {
        ...
        expect(mockPrisma.idempotencyKey.upsert).toHaveBeenCalledWith(expect.objectContaining({
            where: { key: 'key1' },
