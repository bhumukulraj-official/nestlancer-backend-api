...
describe('LocalProvider', () => {
    ...
    it('should upload a file', async () => {
        const result = await provider.upload('test-bucket', 'test.txt', Buffer.from('data'), 'text/plain');

        expect(fs.mkdir).toHaveBeenCalled();
        expect(fs.writeFile).toHaveBeenCalledWith(join('/tmp/storage', 'test-bucket', 'test.txt'), expect.any(Buffer));
    ...
    it('should delete a file', async () => {
        ...
        expect(fs.unlink).toHaveBeenCalledWith(join('/tmp/storage', 'test-bucket', 'test.txt.meta.json'));
    });
