...
describe('DatabaseMetricsCollector', () => {
    ...
    it('should record query', () => {
        ...
        expect(mockHistogram.observe).toHaveBeenCalledWith({ operation: 'findMany', model: 'User' }, 0.05);
    });

    it('should record error', () => {
        collector.recordError('findMany', 'User', 'TimeoutError');
        expect(mockCounter.inc).toHaveBeenCalledWith({ operation: 'findMany', model: 'User', error_type: 'TimeoutError' });
    });
