...
export class NestlancerLoggerService implements LoggerService {
  ...
  debug(message: string, context?: string): void {
    if (this.options.level === 'debug') console.debug(JSON.stringify({ level: 'debug', message, context, timestamp: new Date().toISOString() }));
  }
  verbose(message: string, context?: string): void {
    console.log(JSON.stringify({ level: 'verbose', message, context, timestamp: new Date().toISOString() }));
  }
}
