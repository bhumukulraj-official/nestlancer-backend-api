...
export async function seedTestBlogPosts(prisma: PrismaClient): Promise<void> {
  ...
  const posts = [
    {
      ...
      excerpt:
        'Learn how to build production-ready microservices using NestJS, RabbitMQ, and PostgreSQL.',
      content:
    ...
    },
    {
      ...
      seo: {
        ...
        description:
          'Essential TypeScript patterns and practices for maintaining clean, scalable code.',
      },
  ...
  const firstPost = await prisma.blogPost.findUnique({
    where: { slug: 'building-scalable-microservices-nestjs' },
  });
  if (firstPost) {
    ...
    await prisma.blogComment.upsert({
      ...
      create: {
        ...
        content:
          'Great article! The transactional outbox pattern is crucial for reliable messaging.',
        status: 'APPROVED',
      },
    });
  ...
  await prisma.contactMessage.upsert({
    ...
    create: {
      ...
      subject: 'GENERAL',
      message:
        "I'm interested in building a SaaS application. Could we discuss the project scope and timeline?",
      status: 'NEW',
