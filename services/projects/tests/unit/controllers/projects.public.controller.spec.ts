...
describe('ProjectsPublicController', () => {
    ...
    describe('getPublicProjectDetails', () => {
        it('should throw BusinessLogicException', async () => {
            await expect(controller.getPublicProjectDetails('p1')).rejects.toThrow(BusinessLogicException);
            await expect(controller.getPublicProjectDetails('p1')).rejects.toThrow('Project not public or not found');
        });
    });
});
