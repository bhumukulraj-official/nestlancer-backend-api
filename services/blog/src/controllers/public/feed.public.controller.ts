...
export class FeedPublicController {
    ...
    @Get('rss')
    @Cacheable({ ttl: 3600 })
    @ApiOperation({ summary: 'Get global RSS feed', description: 'Retrieve the latest published blog posts in standard RSS 2.0 format.' })
    ...
    @Get('atom')
    @Cacheable({ ttl: 3600 })
    @ApiOperation({ summary: 'Get global Atom feed', description: 'Retrieve the latest published blog posts in standard Atom format.' })
