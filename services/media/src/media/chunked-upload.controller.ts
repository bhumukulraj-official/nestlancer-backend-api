...
export class ChunkedUploadController {
  ...
  @ApiOperation({
    ...
    description: 'Start a multipart upload session for a large file.',
  })
    ...
    const media = await this.prismaWrite.media.create({
      data: {
        ...
        metadata: {
          ...
          totalChunks: payload.totalSize
            ? Math.ceil(payload.totalSize / (payload.chunkSize || 10485760))
            : 1,
          receivedChunks: [],
          bucket: 'default',
        },
  ...
  @ApiOperation({
    ...
    description: 'Provide a single part of the multipart file upload.',
  })
  async uploadChunk(
    @CurrentUser() user: AuthenticatedUser,
  ...
  @ApiOperation({
    ...
    description: 'Notify the system that all parts have been uploaded.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Verify which chunks have been successfully received.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Cancel the multipart session and discard all received data.',
  })
