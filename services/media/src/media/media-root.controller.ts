...
export class MediaRootController {
     ...
     */
    @Get('health')
    @ApiOperation({ summary: 'Service health', description: 'Simple health check for the media service instance.' })
    ...
    @ApiOperation({ summary: 'Root upload', description: 'Exposes direct upload at the root path for specific gateway configurations.' })
    async upload(
    ...
    @Get('stats')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Get root stats', description: 'Retrieve storage statistics for the user at the root level.' })
    ...
    @Get(':id/status')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Get processing status', description: 'Fetch the real-time processing state of a media asset.' })
