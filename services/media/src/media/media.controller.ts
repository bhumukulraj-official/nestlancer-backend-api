...
export class MediaController {
    ...
    @ApiOperation({ summary: 'List user media', description: 'Retrieve all media files uploaded by the current user with optional filtering.' })
    async getUserMedia(
    ...
    @Post('upload/request')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Request upload', description: 'Initiate an upload session by providing file metadata.' })
    async requestUpload(
        @CurrentUser() user: AuthenticatedUser,
    ...
    @Post('upload/confirm')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Confirm upload', description: 'Notify the system that the file upload to storage is complete.' })
    async confirmUpload(
        @CurrentUser() user: AuthenticatedUser,
    ...
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Direct upload', description: 'Upload a file directly to the server in a single request.' })
    async directUpload(
        @CurrentUser() user: AuthenticatedUser,
    ...
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Get storage stats', description: 'Retrieve current storage usage and limits for the authenticated user.' })
    ...
    @Get(':id')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Get media details', description: 'Fetch detailed metadata and processing status for a specific file.' })
    ...
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Update metadata', description: 'Modify the descriptive metadata associated with a media file.' })
    ...
    @Delete(':id')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Delete media', description: 'Permanently remove a media file and its associated storage assets.' })
    async deleteMedia(
        @CurrentUser() user: AuthenticatedUser,
        @Param('id') id: string,
    ): Promise<any> {
    ...
    @Get(':id/download')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Get download URL', description: 'Generate a secure, time-limited URL for downloading the file.' })
    ...
    @Post(':id/copy')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Copy media', description: 'Create a duplicate of an existing media file.' })
    async copyMedia(
        @CurrentUser() user: AuthenticatedUser,
        @Param('id') id: string,
    ...
    @Post(':id/move')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Move media', description: 'Relocate a media file to a new virtual directory.' })
    async moveMedia(
        @CurrentUser() user: AuthenticatedUser,
        @Param('id') id: string,
        @Body() body: { destinationFolderId: string },
    ...
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'Regenerate thumbnail', description: 'Trigger the re-creation of preview images for this file.' })
    ...
    @Get(':id/versions')
    @ApiStandardResponse(Object)
    @ApiOperation({ summary: 'List versions', description: 'Fetch all historical versions and revisions of this media file.' })
    async getVersions(
        @CurrentUser() user: AuthenticatedUser,
        @Param('id') id: string,
    ): Promise<any> {
