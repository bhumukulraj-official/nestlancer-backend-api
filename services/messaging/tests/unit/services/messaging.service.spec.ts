...
describe('MessagingService', () => {
    ...
    beforeEach(() => {
        mockPrismaRead = {
            message: {
                findMany: jest.fn().mockResolvedValue([{ id: 'msg-1', content: 'Hello', senderId: 'user-1' }]),
                count: jest.fn().mockResolvedValue(1),
                findUnique: jest.fn().mockResolvedValue({ id: 'msg-1', content: 'Hello', senderId: 'user-1', deletedAt: null }),
            },
        };
        mockPrismaWrite = {
            $transaction: jest.fn().mockImplementation(async (fn: any) => {
                const tx = {
                    message: { create: jest.fn().mockResolvedValue({ id: 'msg-new', projectId: 'proj-1', senderId: 'user-1', content: 'Test' }) },
                    outbox: { create: jest.fn().mockResolvedValue({}) },
                };
                return fn(tx);
            }),
            message: {
                update: jest.fn().mockResolvedValue({ id: 'msg-1', content: 'Updated', editedAt: new Date() }),
            },
        };
        mockOutbox = {};
    ...
    describe('sendMessage', () => {
        it('should send a text message', async () => {
            const result = await service.sendMessage('user-1', { projectId: 'proj-1', content: 'Test', type: 'TEXT' } as any);
            expect(result.id).toBe('msg-new');
            expect(mockPrismaWrite.$transaction).toHaveBeenCalled();
        });

        it('should throw for empty text message content', async () => {
            await expect(service.sendMessage('user-1', { projectId: 'proj-1', content: '', type: 'TEXT' } as any))
                .rejects.toThrow();
        });
    });
    ...
    describe('updateMessage', () => {
        ...
        it('should throw when editing someone else\'s message', async () => {
            mockPrismaRead.message.findUnique.mockResolvedValue({ id: 'msg-1', senderId: 'other-user', deletedAt: null });
            await expect(service.updateMessage('user-1', 'msg-1', { content: 'Updated' } as any))
                .rejects.toThrow();
        });
    });
