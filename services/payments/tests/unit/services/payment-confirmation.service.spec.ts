...
describe('PaymentConfirmationService', () => {
    ...
    describe('confirm', () => {
        it('should throw BadRequestException if signature is invalid', async () => {
            ...
            await expect(service.confirm('user1', { paymentIntentId: 'pi_1', externalPaymentId: 'pay_1', signature: 'sig_1' }))
        ...
        it('should return early if payment already completed', async () => {
            razorpayService.verifyPaymentSignature.mockReturnValue(true);
            prismaRead.payment.findUnique.mockResolvedValue({ id: 'p1', status: PaymentStatus.COMPLETED } as any);

            const result = await service.confirm('user1', { paymentIntentId: 'pi_1', externalPaymentId: 'pay_1', signature: 'sig_1' });
        ...
        it('should update payment and create outbox event', async () => {
            razorpayService.verifyPaymentSignature.mockReturnValue(true);
            prismaRead.payment.findUnique.mockResolvedValue({ id: 'p1', status: PaymentStatus.PENDING } as any);
            prismaWrite.payment.update.mockResolvedValue({ id: 'p1', status: PaymentStatus.COMPLETED } as any);
            ...
            expect(outboxService.create).toHaveBeenCalledWith(expect.objectContaining({
