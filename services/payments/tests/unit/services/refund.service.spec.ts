...
describe('RefundService', () => {
    ...
    describe('processRefund', () => {
        it('should throw NotFoundException if payment not found', async () => {
            ...
            await expect(service.processRefund('1', 'admin1', {} as any)).rejects.toThrow(NotFoundException);
        });
        ...
        it('should throw BadRequestException if payment not completed', async () => {
            ...
            await expect(service.processRefund('1', 'admin1', {} as any)).rejects.toThrow(BadRequestException);
        });

        it('should throw BadRequestException if missing externalId', async () => {
            prismaRead.payment.findUnique.mockResolvedValue({ status: PaymentStatus.COMPLETED, externalId: null } as any);
            await expect(service.processRefund('1', 'admin1', {} as any)).rejects.toThrow(BadRequestException);
        });
        ...
        it('should throw BadRequestException if refund amount exceeds total', async () => {
            ...
            await expect(service.processRefund('1', 'admin1', { amount: 60 } as any)).rejects.toThrow(BadRequestException);
        });
        ...
        it('should throw InternalServerErrorException if Razorpay fails', async () => {
            ...
            await expect(service.processRefund('1', 'admin1', {} as any)).rejects.toThrow(InternalServerErrorException);
        });
    });
});
