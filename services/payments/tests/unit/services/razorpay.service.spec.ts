...
describe('RazorpayService', () => {
    ...
    describe('createOrder', () => {
        it('should create an order with amount in subunits', async () => {
            const spy = jest.spyOn((service as any).instance.orders, 'create').mockResolvedValue({ id: 'order_123' });
            const result = await service.createOrder(5000, 'INR', 'rcpt_1');
            expect(result.id).toBe('order_123');
            expect(spy).toHaveBeenCalledWith(expect.objectContaining({ amount: 500000, currency: 'INR' }));
        });

        it('should throw when Razorpay API fails', async () => {
            jest.spyOn((service as any).instance.orders, 'create').mockRejectedValue(new Error('API error'));
    ...
    describe('verifyWebhookSignature', () => {
        it('should verify a valid webhook signature', () => {
            ...
            const expectedSig = crypto.createHmac('sha256', 'test_webhook_secret').update(body).digest('hex');
