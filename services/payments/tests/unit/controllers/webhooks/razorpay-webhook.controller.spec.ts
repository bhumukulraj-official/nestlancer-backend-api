...
describe('RazorpayWebhookController', () => {
    ...
    describe('handleWebhook', () => {
        it('should call webhookService.handleWebhook', async () => {
            ...
            expect(webhookService.handleWebhook).toHaveBeenCalledWith('sig1', 'test', { event: 'payment.captured' });
            expect(result).toEqual({ status: 'ok' });
        });

        it('should throw Error if rawBody is missing', async () => {
            const req = {} as any;
            await expect(controller.handleWebhook('sig1', req, {})).rejects.toThrow('Raw body not found. Make sure rawBody is enabled in NestJS app.');
        });
    });
});
