...
export class PaymentMethodsService {
    ...
    async updateNickname(userId: string, methodId: string, nickname: string): Promise<SavedPaymentMethodResponse> {
        const method = await this.prismaRead.savedPaymentMethod.findFirst({
            where: { id: methodId, userId }
        });
        ...
        switch (data.type) {
            case 'card':
                if (!data.last4 || !data.cardBrand) {
                    throw new BadRequestException('Card details (last4, cardBrand) are required for card payment methods');
                }
                break;
            case 'upi':
                if (!data.upiVpa) {
