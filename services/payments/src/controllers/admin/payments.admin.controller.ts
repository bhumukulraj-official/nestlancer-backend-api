...
import { PaymentStatsService, PaymentReconciliationService } from '../../services/admin-tasks.service';
...
export class PaymentsAdminController {
    ...
    @ApiStandardResponse(Object)
    async getReconciliation(@Query() query: { startDate?: string; endDate?: string; page?: string; limit?: string }): Promise<any> {
        const startDate = query.startDate ? new Date(query.startDate) : undefined;
        const endDate = query.endDate ? new Date(query.endDate) : undefined;
        const page = query.page ? parseInt(query.page, 10) : undefined;
        const limit = query.limit ? parseInt(query.limit, 10) : undefined;
        const report = await this.reconciliationService.getReconciliationReport({ startDate, endDate, page, limit });
