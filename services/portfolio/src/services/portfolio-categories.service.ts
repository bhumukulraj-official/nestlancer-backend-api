...
export class PortfolioCategoriesService {
    ...
    async update(id: string, dto: UpdateCategoryDto) {
        if (dto.name || dto.slug) {
            const slug = dto.slug || (dto.name ? dto.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') : undefined);
            const existing = await this.prismaRead.portfolioCategory.findFirst({
                where: {
    ...
    async delete(id: string, reassignToId?: string) {
        ...
        if (category._count.items > 0) {
            if (!reassignToId) {
                throw new BadRequestException('Cannot delete category with items unless reassignToId is provided');
            }
