...
export class PortfolioPublicController {
    ...
    @Get()
    @Cacheable({ ttl: 3600 })
    @ApiOperation({ summary: 'List published portfolio items', description: 'Fetch all portfolio entries that are marked as published.' })
    ...
    @Get('featured')
    @Cacheable({ ttl: 7200 })
    @ApiOperation({ summary: 'Get featured portfolio items', description: 'Retrieve high-priority portfolio entries intended for the main showcase.' })
    @ApiStandardResponse({ type: PortfolioItemResponseDto, isArray: true })
    ...
    @Get('categories')
    @Cacheable({ ttl: 3600 })
    @ApiOperation({ summary: 'Get all categories', description: 'Fetch a complete list of active categories used to classify portfolio projects.' })
    ...
    @Get('tags')
    @Cacheable({ ttl: 3600 })
    @ApiOperation({ summary: 'Get all unique tags', description: 'Extract a unique set of all technological and thematic tags used in published items.' })
    ...
    @Public()
    @Get('search')
    @ApiOperation({ summary: 'Search portfolio items', description: 'Perform keyword-based search across the entire public portfolio.' })
    @ApiStandardResponse({ type: PortfolioItemResponseDto, isArray: true })
    ...
    @Public()
    @Get('health')
    @ApiOperation({ summary: 'Service health check', description: 'Confirm the availability and health of the portfolio microservice.' })
