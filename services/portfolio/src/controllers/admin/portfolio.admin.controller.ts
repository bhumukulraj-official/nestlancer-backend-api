...
export class PortfolioAdminController {
     ...
     */
    @Get()
    @ApiOperation({ summary: 'List all portfolio items', description: 'Retrieve a global view of all portfolio entries for administrative management.' })
     ...
     */
    @Post()
    @ApiOperation({ summary: 'Create portfolio item', description: 'Initialize and save a new entry in the portfolio repository.' })
    ...
    @Post('reorder')
    @HttpCode(200)
    @ApiOperation({ summary: 'Reorder portfolio items', description: 'Update the global sorting order of portfolio items for the showcase UI.' })
     ...
     */
    @Get('analytics')
    @ApiOperation({ summary: 'Get global analytics', description: 'Fetch system-wide statistics on views, likes, and engagement trends.' })
     ...
     */
    @Get('analytics/:id')
    @ApiOperation({ summary: 'Get item analytics', description: 'Access detailed engagement history and view counts for a specific entry.' })
     ...
     */
    @Get(':id')
    @ApiOperation({ summary: 'Get portfolio item', description: 'Retrieve detailed metadata and content for any portfolio entry.' })
    @ApiParam({ name: 'id', description: 'Portfolio item UUID' })
    @ApiStandardResponse({ type: PortfolioItemResponseDto })
     ...
     */
    @Patch(':id')
    @ApiOperation({ summary: 'Update portfolio item', description: 'Apply changes to the metadata, description, or media associations of an item.' })
    @ApiParam({ name: 'id', description: 'Portfolio item UUID' })
    @ApiStandardResponse({ type: PortfolioItemResponseDto })
     ...
     */
    @Delete(':id')
    @ApiOperation({ summary: 'Delete portfolio item', description: 'Deactivate and mark an entry for removal from the public repository.' })
    @ApiParam({ name: 'id', description: 'Portfolio item UUID' })
