...
export class CreateMilestoneDto {
    @ApiProperty({ example: 'Frontend MVP Development', maxLength: 200, description: 'The name of the milestone' })
    @IsString()
    @MaxLength(200)
    name: string;

    @ApiPropertyOptional({ example: 'Implementation of the core frontend features...', description: 'Detailed description of milestone objectives' })
    @IsOptional()
    @IsString()
    description?: string;
    ...
    @ApiPropertyOptional({ type: [String], example: ['Homepage UI', 'Auth Flow'], description: 'Initial list of deliverable titles' })
    @IsOptional()
    @IsArray()
    deliverables?: string[];
