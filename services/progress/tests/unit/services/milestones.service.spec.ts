...
describe('MilestonesService', () => {
    ...
    beforeEach(() => {
        mockPrismaRead = {
            milestone: {
                findUnique: jest.fn().mockResolvedValue({ id: 'ms-1', projectId: 'proj-1', status: 'PENDING', name: 'Design' }),
        ...
        mockPrismaWrite = {
            milestone: {
                create: jest.fn().mockResolvedValue({ id: 'ms-new', projectId: 'proj-1', name: 'Dev', status: 'PENDING', order: 3 }),
                update: jest.fn().mockResolvedValue({ id: 'ms-1', name: 'Updated Design' }),
            },
            $transaction: jest.fn().mockImplementation(async (fn) => {
                const tx = {
                    milestone: { update: jest.fn().mockResolvedValue({ id: 'ms-1', projectId: 'proj-1', status: 'COMPLETED', name: 'Design', completedAt: new Date() }) },
    ...
    describe('create', () => {
        it('should create a milestone with auto-order', async () => {
            const result = await service.create('proj-1', { name: 'Dev', description: 'Development phase', startDate: '2025-01-01', endDate: '2025-03-01' } as any);
            expect(result.id).toBe('ms-new');
            expect(result.order).toBe(3);
        });

        it('should use provided order', async () => {
            await service.create('proj-1', { name: 'Dev', description: 'Test', startDate: '2025-01-01', endDate: '2025-03-01', order: 1 } as any);
