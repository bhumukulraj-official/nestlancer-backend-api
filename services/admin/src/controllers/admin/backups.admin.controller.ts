...
export class BackupsAdminController {
     ...
     */
    @Get()
    @ApiOperation({ summary: 'List backups', description: 'Fetch a list of all available database backups including their status, size, and creation date.' })
     ...
     */
    @Post()
    @ApiOperation({ summary: 'Trigger backup', description: 'Manually initiate a new database backup with optional notes and configuration.' })
     ...
     */
    @Get(':id')
    @ApiOperation({ summary: 'Get backup details', description: 'Retrieve comprehensive metadata for a specific database backup.' })
     ...
     */
    @Get(':id/download')
    @ApiOperation({ summary: 'Download backup', description: 'Generate a secure, time-limited download link for a database backup file.' })
     ...
     */
    @Post(':id/restore')
    @ApiOperation({ summary: 'Restore backup', description: 'Restore the system database to a previous state using a specific backup file.' })
    @SuccessResponse('Backup restore initiated')
    async restore(@Param('id') id: string, @Body() dto: RestoreBackupDto, @Req() req: any): Promise<any> {
        dto.backupId = id;
     ...
     */
    @Get('schedule')
    @ApiOperation({ summary: 'Get backup schedule', description: 'Retrieve the current configuration for automated recurring database backups.' })
     ...
     */
    @Patch('schedule')
    @ApiOperation({ summary: 'Update backup schedule', description: 'Modify the frequency, timing, and retention rules for automatic database backups.' })
