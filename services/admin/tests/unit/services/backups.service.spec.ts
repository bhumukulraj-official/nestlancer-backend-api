...
describe('BackupsService', () => {
  ...
  describe('createBackup', () => {
    it('should create a backup and publish a queue message', async () => {
      ...
      expect(queueService.publish).toHaveBeenCalledWith('admin', 'DATABASE_BACKUP_CREATE', {
    ...
    it('should throw Error if creation fails', async () => {
      ...
      await expect(service.createBackup({}, 'admin1')).rejects.toThrow(
  ...
  describe('restoreBackup', () => {
    it('should publish restore message if backup is COMPLETED', async () => {
      ...
      expect(queueService.publish).toHaveBeenCalledWith('admin', 'DATABASE_BACKUP_RESTORE', {
        backupId: '1',
        adminId: 'admin1',
      });
    ...
    it('should throw InternalServerErrorException if backup is not COMPLETED', async () => {
      ...
      const dto: RestoreBackupDto = { backupId: '1' };
      await expect(service.restoreBackup(dto, 'admin1')).rejects.toThrow(
        InternalServerErrorException,
      );
    });
  });
