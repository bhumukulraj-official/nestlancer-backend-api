...
describe('RequestsAdminService', () => {
    ...
    const mockRequest = {
        id: 'req-1', title: 'Build website', description: 'Full website', status: 'SUBMITTED',
        budgetMin: 1000, budgetMax: 5000, budgetCurrency: 'INR',
        user: { id: 'user-1', email: 'test@example.com', firstName: 'John', lastName: 'Doe' },
        category: 'WEB_DEVELOPMENT', createdAt: new Date(), updatedAt: new Date(), submittedAt: new Date(),
        attachments: [], quotes: [], statusHistory: [], adminNotes: [],
        preferredStartDate: null, deadline: null, requirements: null, technicalRequirements: null,
    };
    ...
    beforeEach(() => {
        ...
        mockPrismaWrite = {
            $transaction: jest.fn().mockImplementation(async (fn) => {
                const tx = {
                    projectRequest: { update: jest.fn().mockResolvedValue({ ...mockRequest, status: 'UNDER_REVIEW', updatedAt: new Date() }) },
    ...
    describe('updateRequestStatus', () => {
        it('should update status with transaction', async () => {
            const result = await service.updateRequestStatus('req-1', 'admin-1', 'underReview', 'Reviewing now');
            expect(result.id).toBeDefined();
            expect(mockPrismaWrite.$transaction).toHaveBeenCalled();
        });

        it('should throw for invalid status transition', async () => {
            mockPrismaRead.projectRequest.findFirst.mockResolvedValueOnce({ ...mockRequest, status: 'QUOTED' });
            await expect(service.updateRequestStatus('req-1', 'admin-1', 'submitted'))
                .rejects.toThrow();
        });
    });
