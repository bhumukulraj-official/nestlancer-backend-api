...
describe('QuotesAdminService', () => {
    ...
    beforeEach(async () => {
        ...
        prismaWrite.$transaction.mockImplementation(async (cb: any) => {
            return cb({
                quote: { create: jest.fn().mockResolvedValue({ id: 'q1', requestId: 'req1', createdAt: new Date(), items: [] }) },
    ...
    describe('createQuote', () => {
        ...
        it('should throw BusinessLogicException if request not found', async () => {
            ...
            await expect(service.createQuote('req1', 'admin1', dto)).rejects.toThrow(BusinessLogicException);
        });

        it('should throw BusinessLogicException if request is already QUOTED', async () => {
            prismaRead.projectRequest.findFirst.mockResolvedValue({ id: 'req1', status: 'QUOTED' } as any);
            await expect(service.createQuote('req1', 'admin1', dto)).rejects.toThrow(BusinessLogicException);
        });

        it('should create a quote via transaction and return formatted quote', async () => {
            prismaRead.projectRequest.findFirst.mockResolvedValue({ id: 'req1', userId: 'user1', status: 'PENDING' } as any);
