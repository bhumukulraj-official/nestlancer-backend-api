    ...
    describe('Admin Debug (Authenticated)', () => {
        it('GET /api/v1/health/debug (Anonymous should fail)', async () => {
            ...
            if (response.status !== 401 && response.status !== 500) {
                console.error('Anonymous debug access should fail with 401 (or 500 error filter), but got:', response.status, response.body);
            }
        ...
        it('GET /api/v1/health/debug (User should fail)', async () => {
            ...
            if (response.status !== 403) {
                console.error('User debug access should fail with 403, but got:', response.status, response.body);
            }
