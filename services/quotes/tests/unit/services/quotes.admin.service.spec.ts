...
describe('QuotesAdminService', () => {
    ...
    beforeEach(() => {
        mockPrismaRead = {
            quote: {
                findMany: jest.fn().mockResolvedValue([{ id: 'q1', status: 'SENT', totalAmount: 5000, currency: 'INR', createdAt: new Date(), user: { firstName: 'John', email: 'test@example.com' } }]),
                count: jest.fn().mockResolvedValue(1),
                findUnique: jest.fn().mockResolvedValue({ id: 'q1', userId: 'user-1', status: 'DRAFT' }),
            },
            projectRequest: { findUnique: jest.fn().mockResolvedValue({ id: 'req-1', userId: 'user-1' }) },
        };
        mockPrismaWrite = {
            quote: { create: jest.fn().mockResolvedValue({ id: 'q-new', status: 'DRAFT', totalAmount: 5000, currency: 'INR' }) },
    ...
    describe('createQuote', () => {
        it('should create a quote for a valid request', async () => {
            const dto = { requestId: 'req-1', totalAmount: 5000, currency: 'INR', validUntil: new Date().toISOString(), terms: 'Standard', notes: '', paymentBreakdown: [{ milestone: 'Advance', amount: 5000 }] };
        ...
        it('should throw for non-existent request', async () => {
            ...
            await expect(service.createQuote('admin-1', { requestId: 'invalid' } as any)).rejects.toThrow();
        });

        it('should throw when payment breakdown total mismatch', async () => {
            const dto = { requestId: 'req-1', totalAmount: 5000, currency: 'INR', validUntil: new Date().toISOString(), paymentBreakdown: [{ milestone: 'Advance', amount: 3000 }] };
