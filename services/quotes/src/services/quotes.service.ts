...
import { QuoteStatus } from '@nestlancer/common';
...
export class QuotesService {
    ...
    private formatQuoteDetailResponse(quote: any): QuoteDetail {
        ...
        const technicalDetails = this.parseJsonField<Record<string, unknown>>(quote.technicalDetails, {});
        ...
        return {
            ...
            technicalDetails,
            terms: quote.terms,
            attachments: this.parseJsonField<Array<{ id: string; name: string; url: string }>>(quote.attachments, []),
            request: {
                id: quote.request?.id,
    ...
    private buildPaymentBreakdown(quote: any): Array<{ milestone: string; amount: number; percentage: number; dueOn: string }> {
        const breakdown = this.parseJsonField<Array<{ milestone: string; amount: number; percentage: number; dueOn: string }>>(
    ...
    private buildStatusHistory(quote: any): Array<{ status: string; timestamp: Date; actor?: string }> {
        ...
        if (quote.status === QuoteStatus.VIEWED || quote.status === QuoteStatus.ACCEPTED || quote.status === QuoteStatus.DECLINED) {
