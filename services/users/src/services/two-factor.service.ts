...
export class TwoFactorService {
    ...
    async verify2FASetup(userId: string, dto: { code: string }): Promise<any> {
        ...
        if (!authConfig?.twoFactorSecret) {
            throw new BusinessLogicException('2FA setup not initiated. Call enable2FA first.', 'USER_017');
        }
    ...
    async regenerateBackupCodes(userId: string, dto: { password: string }): Promise<any> {
        ...
        if (!user.twoFactorEnabled) {
            throw new BusinessLogicException('2FA must be enabled to regenerate backup codes', 'USER_019');
        }
