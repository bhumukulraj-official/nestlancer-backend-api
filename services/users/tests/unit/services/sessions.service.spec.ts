...
describe('SessionsService', () => {
  ...
  describe('terminateSession', () => {
    ...
    it('should throw when trying to terminate current session', async () => {
      ...
      await expect(
        service.terminateSession('user-1', 'session-1', 'current-jti'),
      ).rejects.toThrow();
    });
    ...
    it('should throw for non-existent session', async () => {
      ...
      await expect(service.terminateSession('user-1', 'invalid', 'current-jti')).rejects.toThrow();
    });

    it('should throw for session belonging to another user', async () => {
      mockPrismaRead.session.findUnique.mockResolvedValue({
        ...mockSessions[1],
        userId: 'other-user',
      });

      await expect(
        service.terminateSession('user-1', 'session-2', 'current-jti'),
      ).rejects.toThrow();
    });
  });
