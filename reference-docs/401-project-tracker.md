...
- [x] `notification-worker/` – Consumes `notification.queue`. Sends push notifications via FCM/APNS, SMS via Twilio, publishes to Redis pub/sub for WebSocket delivery. Scale based on notification queue depth.
